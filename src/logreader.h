...
public:
    ...
    static SVec2 GetNormalizedPosition(const SVec2& pos, const SAABB& aabb);
...
private:
    ...
    static std::vector<std::string> SplitString(const std::string& input,
                                                char delimiter = ',');
    static std::vector<float> SplitFloats(const std::string& input,
                                          char delimiter = ',');
...
private:
    ...
    std::vector<size_t> m_vLapStartIndex;  // Indicates the starting and end
                                           // index of the record in each lap
