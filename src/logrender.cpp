...
void LogRender::DrawAcceBox()
{
    ...
    draw_list->AddRectFilled(ImVec2(x - SZ, y), ImVec2(x + SZ, y + 1.0f), 0xFFFFFFFF);
      // Horizontal line (faster than AddLine, but only handle
                        // integer thickness)
    draw_list->AddRectFilled(ImVec2(x, y - SZ), ImVec2(x + 1.0, y + SZ), 0xFFFFFFFF);
      // Vertical line (faster than AddLine, but only handle
                          // integer thickness)
